...
export class NavContentComponent implements OnInit {
  ...
  // version
  title = 'home application for version numbering';
  ...
  constructor() {
    ...
    this.scrollWidth = 0;
    this.contentWidth = 0;
    this.navigation = this.filterNavigationItems(NavigationItems);
  }
  ...
  private filterNavigationItems(items: NavigationItem[]): NavigationItem[] {
    ...
    return items.reduce<NavigationItem[]>((filtered, item) => {
      const allowedRoles = item.allowedRoles?.map((role) => role.toLowerCase()) ?? [];
      ...
      const nextItem: NavigationItem = { ...item };
      if (nextItem.children) {
        nextItem.children = this.filterNavigationItems(nextItem.children);
